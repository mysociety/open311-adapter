...
subtest "invalid contact from Alloy is ignored" => sub {
    my $result = $endpoint->_search_for_code_by_argument({ search => 'error@example.com', dodi_code => $endpoint->config->{contact}->{code}, attribute => $endpoint->config->{contact}->{search_attribute_code_email}});
    is $result, undef, "User doesn't exist";
};
