...
my $dir = path(__FILE__);
my $files = $dir->sibling('files')->child('bexley');
$bexley_end->mock('_build_config_file', sub {
    $dir->sibling('bexley_symology.yml');
});
$bexley_end->mock(_get_update_files => sub { [ $files->child('file1.csv'), $files->child('file2.csv') ] });
